...
//! variable overwrite. RELEASE ONLY (3-way equality gate).
...
fn main() {
    ...
    let mut check = |s: &str| {
        let h = intern_hash(s);
        match seen.get(&h) {
            Some(prev) => assert_eq!(prev.as_str(), s, "hash collision `{prev}` vs `{s}` (would bail)"),
            None => {
    ...
    // Correctness gate: naive == JIT-off == JIT-on (all read the same rows).
    ...
    println!();
    println!(
        "  => JIT-off is {:.2}x of naive, so the {:.0}x win is COMPILATION + interning, not lowering.",
        jit_off / nv,
        nv / jit
    );
